...
const INTENT_RULES: Record<string, IntentRule> = {
  coalition_proposal: {
    ...
    severity: 4,
    guidance: '这是执政党试图拉拢在野党加入联盟的场景。体现政治博弈、利益交换和权力平衡。场景应为符合高层政治谈判的封闭、官方或半官方场所。',
  },
  cabinet_reshuffle: {
    description: '首相进行内阁改组',
    severity: 3,
    guidance: '首相对内阁进行人事调整，可能涉及撤换、提拔、安抚联盟伙伴等。场景应为符合高层人事决策的官方场所（如首相官邸、党干部室）。',
  },
  opposition_attack: {
  ...
  opposition_coalition: {
    ...
    severity: 4,
    guidance: '两大在野党秘密碰面商讨合作。如果玩家是执政党，则是撞破密会获得情报；如果在野党，则是受邀参加盟友密会。场景应为符合密会性质的隐秘、私密地点，由剧情自然决定。',
  },
  coalition_pressure: {
    description: '执政联盟内小党向首相施压',
    severity: 3,
    guidance: '执政联盟内部出现利益分配矛盾，小党要求更多内阁职位或政策话语权。场景应为执政党内部博弈的封闭场所（如党本部、首相办公室、干部会议室）。',
  },
  faction_challenge: {
    description: '党内出现挑战党首的派系',
    severity: 4,
    guidance: '党内某位有野心的议员公开或暗中挑战现任党首的领导地位，党面临分裂危机。场景可以是党大会、党干部会议、记者会等。',
  },
  media_boost: {
    description: '媒体对某党进行了正面报道',
    severity: 2,
    guidance: '媒体对某党进行了有利报道，提升了公众形象。场景应聚焦媒体机构本身（编辑部、演播室、记者会等）。',
  },
  media_scandal: {
    description: '媒体曝光某党的丑闻',
    severity: 4,
    guidance: '媒体爆出某党的负面新闻（政治献金、腐败、丑闻等），对该党造成严重打击。场景应聚焦媒体机构本身（新闻直播间、调查报道现场、记者会等）。',
  },
  lobby_funds: {
    description: '利益集团向政党提供政治捐款',
    severity: 3,
    guidance: '经济团体（经团连等）接触政党议员，以捐款换取政策支持。这是一个敏感的政治交易场景。场景应为符合私下交易的封闭、私密场所。',
  },
  // --- Phase G Q6：6 个程序性 intent ---
  no_confidence_proposal: {
    description: '在野党正式提出内阁不信任动议',
    severity: 5,
    guidance: '在野党达到联署门槛后正式提出不信任动议，倒阁博弈白热化。场景在众议院议事厅，党首选边站队，首相可能选择辞职或解散众议院。',
  },
  dissolution_decision: {
    description: '首相考虑解散众议院',
    severity: 5,
    guidance: '首相在战略时刻考虑解散众议院，重新大选。场景在首相官邸或内阁会议，权衡支持率、在野党分散度、任期剩余。这是高风险高回报的政治赌博。',
  },
  leadership_campaign: {
    description: '党内启动党首选举',
    severity: 4,
    guidance: '党内因支持率低迷、丑闻、派阀挑战等原因启动党首选举。候选人辩论、派阀站队、密室交易。场景在党本部或党大会。',
  },
  bill_draft: {
    description: '议员或党派起草新法案',
    severity: 2,
    guidance: '法案起草阶段，议员与党派智囊商议条款、目标委员会、政治意图。场景在议员办公室或党政策委员会。',
  },
  parliament_questioning: {
    description: '国会议员质询内阁大臣',
    severity: 3,
    guidance: '国会质询环节，在野党议员质询执政党大臣，质询时间按会派席位比例分配。场景在众议院议事厅，公开直播，媒体密切关注。',
  },
...
4. 场景必须自然贴合事件性质的真实发生地（议事堂、党部、办公室、媒体现场、私密空间等任选），避免固定套路和重复
...
## 政治体系参考（叙事生成依赖规则）
### 议会结构
- 众议院 200 席 = 110 直接（11 大选区 × 10 席）+ 90 全国比例代表（D'Hondt，5% 阈值），并行制（Phase G Q1）
...
### 政治晋升（双轨制，Phase G Q2）
- 党内（8 级）：普通党员→党务干部→政策委员会委员→政策委员会委员长→副干事长→干事长→副党首→党首
- 国会（9 级）：普通议员→委员会理事→副委员长→委员长→政务官→副大臣→国务大臣→内阁官房长官→总理大臣
- 党派别名：联合工人党（solidarity）用"主席/副主席/书记局长/副书记/组织局干部"替代党首/副党首/干事长/副干事长/党务干部；改革民主党/保守党的"政策委员会委员长"显示为"政调会长"
- 晋升阈值（党内）：loyalty>70 + 政治资本>30 + 党龄>6 回合
- 晋升阈值（国会）：资历>8 回合 + 委员会成绩>60
